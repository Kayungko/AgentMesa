...
| **Core integration** | **Partial.** `assertPolicy` calls `ctx.policy.can()` in all state-changing services. Public projection read APIs (`getTaskProjection`, `listTaskProjections`, etc.) enforce `projection.read`; internal `_get*`/`_list*` helpers are excluded from `@agentmesa/core` exports and only used by `read-model-service` to avoid double-enforcement. Default remains `AllowAllMesaPolicyEngine` for local dev; production mode should use `role-based`. Capability gating (canEditFiles, canRunShell, etc.) is not yet checked by core services. |
