...
| **Atomic lock creation** | **Done.** `acquireLock` uses the `wx` open flag so lock-file creation is atomic — no check-then-write race. Lock filenames are sha256 hashes of the resource id, keeping them filesystem-safe and traversal-proof; the original resource is stored in the lock body. Each lock carries a unique UUID `token` for precise holder identification. |
...
| **Persistent event store** | **Done.** `FileEventStore` appends each event to `.agentmesa/events/events.jsonl`, validates every line against `MesaEventSchema` on read, and `fsync`s after each append. `appendRuntimeEvent` wraps sequence allocation + append in `withLock('event_log', ...)` so concurrent clients cannot produce duplicate sequences. |
| **Event log & projection diagnostics** | **Done.** `validateEventLog` scans `events.jsonl` for corrupt JSON and invalid events. `checkProjectionConsistency` flags event streams without projections. `findOrphanedLocks` detects lock files whose owning PID is dead. All wired into `mesa doctor`. |
| **Projection rebuild** | **Done with known limits.** `projection-service.ts` replays task/meeting/agent events, writes rebuilt projections to `.agentmesa/projections/`. Supports `{ clean: true }` for stale projection removal. Replay is deterministic (sequence → timestamp → id). `projection-read-service.ts` provides `get*`/`list*` helpers and `is*ProjectionFresh` freshness checks. `archiveTask` emits `task_archived` (same tombstone semantics as `task_deleted`). **Limits:** (1) no incremental rebuild; (2) no staleness auto-detection on read. |
| **Authoritative read model** | **Done (hardened).** `read-model-service.ts` provides `get*ReadModel`/`list*ReadModels` for tasks, meetings, and agents. Read mode is controlled by `config.readModel.mode`: `projection` (strict — throws `MesaError` on missing/stale/corrupt, no fallback), `hybrid` (default — fresh projection, else warn + legacy fallback), `legacy` (never reads projections). Both GET and LIST operations enforce freshness: projection mode throws on any stale entry, hybrid mode warns and falls back to legacy. All modes are tested. The CLI `show`/`list` commands use the read-model-service. |
| **Projection freshness check** | **Done.** `isTaskProjectionFresh`, `isMeetingProjectionFresh`, `isAgentProjectionFresh` compare `_meta.lastSequence` against the max event sequence for the stream. Return `false` when projection is missing or stale. Stale projections are reported by `mesa doctor` as fixable warnings. |
| **CLI rebuild command** | **Done.** `mesa rebuild` rebuilds all projections from events with `{ clean: true }` by default. Supports `--json` and `--no-clean` flags. |
| **Lock timeout & retry** | **Done.** `acquireLock` accepts `{ timeoutMs?, retryIntervalMs? }`; polls at retryIntervalMs until timeoutMs expires; throws `LockError` with "timed out" message on expiry. `withLock` forwards options. Default `timeoutMs=0` preserves backward-compatible immediate throw. |
| **Lock token validation** | **Done.** `acquireLock` returns the UUID token string (not void). `releaseLock` validates the token before releasing; throws `LockError` on mismatch. `releaseLockUnsafe` provides backward-compatible unsafe release without token validation. `withLock` internally holds the token and passes it to `releaseLock`. |
