...
| **Projection rebuild** | **Done with known limits.** `projection-service.ts` replays task/meeting/agent events, writes rebuilt projections to `.agentmesa/projections/`. Supports `{ clean: true }` for stale projection removal. Replay is deterministic (sequence → timestamp → id). `projection-read-service.ts` provides `get*`/`list*` helpers. `archiveTask` emits `task_archived` (same tombstone semantics as `task_deleted`). **Limits:** (1) Existing services still read from `.agentmesa/tasks/` etc. — projections are not yet the authoritative read path; (2) no incremental rebuild; (3) no staleness auto-detection on read. |
