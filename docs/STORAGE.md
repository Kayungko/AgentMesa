...
| **Persistent event store** | **Done.** `FileEventStore` appends each event as a single-line JSON object to `.agentmesa/events/events.jsonl`. Runtime events survive process exit and are readable across subsequent `createRuntimeContext` calls. Projection rebuild (reconstructing current-state JSON from the event log) is not yet implemented. |
