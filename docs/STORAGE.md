...
| **Lock timeout & retry** | **Not yet.** `acquireLock` currently checks PID once and throws on contention — no configurable timeout, no retry loop, no `waitForLock(ms)` API. A live holder blocks other clients immediately rather than waiting for release. |
