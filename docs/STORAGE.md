...
| **Persistent event store** | **Done** with known limits. `FileEventStore` appends each event to `.agentmesa/events/events.jsonl` and validates every line against `MesaEventSchema` on read. **Limits:** (1) `appendFileSync` does not `fsync` per event — an OS crash may lose the last write; (2) `appendRuntimeEvent` derives `sequence` from `list().length` before append — not concurrency-safe across multiple clients. |
