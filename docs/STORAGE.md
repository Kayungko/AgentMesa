...
| **Projection rebuild** | **Minimal version done.** `projection-service.ts` replays task and meeting events, writes rebuilt projections to `.agentmesa/projections/`. Deleted tasks produce tombstone projections. Existing services still read from `.agentmesa/tasks/` etc. — projections are not yet the authoritative read path. |
