...
| **Persistent event store** | **Done** with known limits. `FileEventStore` appends each event to `.agentmesa/events/events.jsonl` and validates every line against `MesaEventSchema` on read. **Limits:** (1) `appendFileSync` does not `fsync` per event — an OS crash may lose the last write; (2) `appendRuntimeEvent` derives `sequence` from `list().length` before append — not concurrency-safe across multiple clients; (3) projection rebuild is not yet implemented. Events are now a durable audit log on disk, but not yet the sole source of truth. |
