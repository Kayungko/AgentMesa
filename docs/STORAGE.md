...
| **Authoritative read model** | **Done.** `read-model-service.ts` provides `get*ReadModel`/`list*ReadModels` for tasks, meetings, and agents. Read mode is controlled by `config.readModel.mode`: `hybrid` (projection with legacy fallback, default), `projection` (projections only), `legacy` (backward compatible). The CLI `show`/`list` commands use the read-model-service. `mesa agent show <id>` subcommand added. |
